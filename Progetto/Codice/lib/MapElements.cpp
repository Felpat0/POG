    ...
    //std::cout<<"\nfine 2\n";
        ...
        if(straightX != 0){
            ...
            do{
                ...
                do{
                    //std::cout<<(m[k + t*oldInc][x + c*inc]);
                    if(m[k + t*oldInc][x + c*inc] == 'X' || m[k + t*oldInc][x + c*inc] == 'D')
                        wallsCounter ++;
                    else
                        wallsCounter = 0;
                    if(wallsCounter > 1)
                        break;
                    c++;
                }while((x + c*inc <= MAX_MATRIX_WIDTH -1 && x + c*inc > -1) && (x + c*inc <= room2.x || x + c*inc >= room2.x + room2.w -1));
            ...
            for(y = k; y != k + t*oldInc; y = y + t*oldInc){
                if(m[y][x] == ' ')
                    m[y][x] == 'O';
            }
            y = k + t*oldInc;
